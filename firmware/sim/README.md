...
sequence** (ROM `0x0600`–`0x074C`). Driven by the `Makefile` one level up
(`firmware/Makefile`).

For prerequisites see `../INSTALL.md`; for the full build/test workflow and the
rationale behind the tests see `../BUILD.md`. This file just documents what
...
| `gen_init.py` | tool | Regenerates `init.a51` from `../bin/M2764A@DIP28.BIN`. |
...
gen_init.py  --(make gen)-->  init.a51
                       ...
                       (make assemble/link/bin)
                                 v
                       build/init.bin  ==  bin/M2764A@DIP28.BIN[0x0600..]   <- golden (make verify)

hex/M2764A@DIP28.HEX --(copied)--> build/rob3.hex --(s51)--> tests/*.sh      <- behavioral (make sim-init / sim-run)
```
...
```bash
make gen        # calls: python3 sim/gen_init.py <rom.bin> sim/init.a51 0x0600 333
```
...
- **Shell-safe HEX.** The shipped image `../hex/M2764A@DIP28.HEX` has an `@` in
  its name. The Makefile copies it to `build/rob3.hex`; simulator commands use
  that copy. Don't pass the `@` path directly to `s51`.
