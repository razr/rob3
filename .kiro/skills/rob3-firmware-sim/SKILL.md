...
REGION=<name>` to check one region, and `make status` to verify all regions
individually.

> **disasm51 install** (environment is externally-managed, so use a venv):
> `python3 -m venv /tmp/d51venv && /tmp/d51venv/bin/pip install disasm51`, then
> `/tmp/d51venv/bin/python d51_to_sdas.py <rom.bin> <org_hex> <len_dec>`.

> **History:** the golden layer previously used per-region `.a51` `.db`
> transcriptions (`init.a51`, `teachbox_scan.a51`) generated by `gen_init.py`.
> Those and `make gen` were removed once the annotated tree became the single
> assembling 1:1 source; the golden test now covers the whole 8 KB ROM.
...
| timer0   | `0x0080` | `timer0_tick.asm`  |
| ext1     | `0x00C0` | `ext1_servo.asm`   |
| rs232    | `0x0300` | `rs232.asm`        |
| init     | `0x0600` | `init.asm`         |
| main     | `0x074D` | `main.asm`         |
| program  | `0x0803` | `program.asm`      |
| teachbox | `0x0C00` | `teachbox.asm`     |
| tables   | `0x0FC5` | `tables.asm`       |

Equates: `inc/{sfr,devices,system,servo,teachbox,serial,program}.inc`;
`system.inc` is the authoritative IRAM map + shared flag-bit (0x20/0x23/0x28)
definitions. Some region bodies are annotated but not yet byte-complete (see
`make status`); completing them to a full 1:1 image is ongoing.
...
- **Golden test FAIL after edits** → you changed bytes; the assembling body in
  the `.asm` file must produce the exact ROM bytes. Use `make verify-region
  REGION=<name>` to test one region, or regenerate the body from the ROM via
  `d51_to_sdas.py` (needs `disasm51` in a venv: `pip install disasm51`).

## When to use this skill

- Running `make verify` / `make sim-*` / `make status`, or adding sim tests.
