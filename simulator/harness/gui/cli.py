...
Run:
    UCSIM_51=/path/to/ucsim_51 python3 cli.py        # firmware in the loop
    python3 cli.py                                   # plant-only fallback
...
def main():
    ...
    ap.add_argument("--firmware", action="store_true",
                    help="drive the real ROM in ucSim (needs the custom ucsim_51; "
                         "slower — interactive ucSim over a pty)")
    args = ap.parse_args()
    cli = TeachboxCLI(step=args.step, drive_firmware=args.firmware)
    if args.script:
