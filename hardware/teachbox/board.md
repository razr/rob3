...
```
   ...
   pin 22 DB25  A2 --| 3   14 |-- /Y1  OUT LED
...
- The 5 Digital Inputs (DI 1–5): the scan routine drives the row strobe by writing
  to the 8255 (DPH=0x51 / 5100H), then reads the resulting column state back on
  the 8031's own **Port 1 (P1, SFR 0x90)** — not on an 8255 port. [verified in main.asm]

```asm
jump_0BFF:
	mov R7, A
	jbc 20h.1, jump_0C64
	mov 83h, #51h	; DPH = 51h -> 8255 Port B (5100H): drive the row strobe
	mov R6, #00h	; Clear data buffer
	mov A, 47h
	anl A, #0Fh	; Isolate matrix mask
jump_0C0C:
	movx @DPTR, A   ; write strobe out through the 8255
	mov 46h, A
	mov A, 90h      ; <--- READ COLUMN RETURNS FROM P1 (SFR 0x90), not the 8255
	anl A, #0E0h	; isolate the column bits
	; ...
```
...
jump_0BFF:
    mov R7, A
    jbc 20h.1, jump_0C64
    mov 83h, #51h    ; DPH = 5100H (8255 Port B): drive the row strobe
    mov R6, #00h     ; Reset key index counter
    ...
jump_0C0C:
    movx @DPTR, A    ; write the strobe out through the 8255
    mov 46h, A
    mov A, 90h       ; read the column returns from P1 (SFR 0x90)
```

- What happens here: The software strobes each row by writing to the 8255
  (Port B, 5100H) which drives the T74LS138B1 lines, then reads the column
  returns on the 8031's **P1 (0x90)** to see which column dropped. [verified]
...
* ST T74LS138 (3-to-8 decoder) datasheet: https://www.st.com/resource/en/datasheet/m74hc138.pdf
* See also: `led.md` (LED decode map), `test.md` (Arduino bring-up test that
  verifies the DB25 address/strobe pins), `pb86-button.md` (key switch).
